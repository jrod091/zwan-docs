...
##DNS Access Control
export DCLAN_INTERFACE="LAN00"
export DCLAN_DNS_AC="172.3.0.0/24"
export TUN00_DNS_AC="10.130.1.0/24"
export TUN01_DNS_AC="10.140.1.0/24"

export DNS_FILTER_REPO="https://zwandemo.amzetta.com:7080/minio/dns"
...
if [ ${IPSEC} == 1 ]; then
    ...
    if [[ "${RES}" =~ "ERROR" ]]; then
        ...
        if [ -n "${result}" ] && [ ${result} == "IPSEC01" ]; then
            ...
            curl -X POST http://127.0.0.1:8765/graphql -H "Content-Type:application/json" --max-time 600 -d '{"variables":{"description":"DC via WAN01","tunnelUUID":"IPSEC01viaWAN01","left":"'${WAN01_localIP}'","right":"0.0.0.0","gateway":"0002","leftSubnet":"'${TUN01_LEFT_SUBNET}'","rightSubnet":"0.0.0.0/0","authType":"X509","type":"ESP","encrptionAlg":"aes256gcm16","integrityAlg":"sha512","dhGroup":"modp4096","lifeTime":"1h","ikeVersion":"ikev2","ikeEncrptionAlg":"aes256gcm16","ikeIntegrityAlg":"sha256","ikeDHGroup":"modp3072","ikeLifetime":"3h","dpdDelay":"30s","dpdTimeout":"150s","dpdAction":"restart","compress":"no","forceUdpEncaps":"yes","leftCert":"01","leftCertId":"'"${LEFT01_CERTID}"'","rightCertId":""}, "query":"mutation createIPsec($description: String!, $tunnelUUID: String!, $type: TunnelType!, $encrptionAlg: String!, $integrityAlg: String!, $dhGroup: String!, $leftSubnet: String, $rightSubnet: String, $lifeTime: String!, $ikeVersion: IKEversion!, $ikeMode: IKEmode, $ikeEncrptionAlg: String!, $ikeIntegrityAlg: String!, $ikeDHGroup: String!, $ikeLifetime: String!, $dpdDelay: String!, $dpdTimeout: String!, $dpdAction: DPDAction!, $left: String!, $right: String!, $authType: AuthenticationType!, $secrets: String, $leftCert: String, $leftCertId: String, $rightCertId: String, $compress: SetValue, $forceUdpEncaps: SetValue, $gateway: String!) {  createIPsec(description: $description, tunnelUUID: $tunnelUUID, type: $type, encrptionAlg: $encrptionAlg, integrityAlg: $integrityAlg, dhGroup: $dhGroup, leftSubnet: $leftSubnet, rightSubnet: $rightSubnet, lifeTime: $lifeTime, ikeVersion: $ikeVersion, ikeMode: $ikeMode, ikeEncrptionAlg: $ikeEncrptionAlg, ikeIntegrityAlg: $ikeIntegrityAlg, ikeDHGroup: $ikeDHGroup, ikeLifetime: $ikeLifetime, dpdDelay: $dpdDelay, dpdTimeout: $dpdTimeout, dpdAction: $dpdAction, left: $left, right: $right, authType: $authType, secrets: $secrets, leftCert: $leftCert, leftCertId: $leftCertId, rightCertId: $rightCertId, compress: $compress, forceUdpEncaps: $forceUdpEncaps, gateway: $gateway) {  code  success  message  ipsec {  description  __typename  }  __typename  } } "}'
...
if [[ "${RES}" =~ "ERROR" ]]; then
    ...
    if [ -n "${result}" ] && [ ${result} == "true" ]; then
        ...
        vEC_LAN_NETWORK="${DCLAN_DNS_AC},${vEC_LAN_NETWORK}"
        ...
        curl -X POST http://127.0.0.1:8765/graphql -H "Content-Type:application/json" --max-time 600 -d '{"variables":{"controllerMode":"CUSTOM","ingressInterface":["WAN00","WAN01"],"egressInterface":["'${TUN00}'","'${TUN01}'","'${DCLAN_INTERFACE}'"]}, "query":"mutation setIPSInterface($controllerMode: IPSControllerMode!, $ingressInterface: [String!], $egressInterface: [String!]) {  setIPSInterface(controllerMode: $controllerMode, ingressInterface: $ingressInterface, egressInterface: $egressInterface) {  code  success  message  __typename  } } "}'
    fi
fi
...
if [[ "${RES}" =~ "ERROR" ]]; then
	...
	if [ -z "${DNS_DOMAINNAME}" ] || [[ -n ${DNS_DOMAINNAME} && ${DNS_DOMAINNAME} != "localserver.com" ]]; then
        ...
        curl -X POST http://127.0.0.1:8765/graphql -H "Content-Type:application/json" --max-time 600 -d '{"variables":{"dnsConfig":{"domainName":"localserver.com","inputIface":["'${DCLAN_INTERFACE}'","'${TUN00}'","'${TUN01}'"],"configStatus":true,"isTLS":true,"dnsSEC":true}}, "query":"mutation addDNS($dnsConfig: addDNSServerConfigInput!) {  addDNS(config: $dnsConfig) {  code  message  success  __typename  } } "}'

        curl -X POST http://127.0.0.1:8765/graphql -H "Content-Type:application/json" --max-time 600 -d '{"variables":{"tlsStatus":true}, "query":"mutation enableDisableTLSForward($tlsStatus: Boolean!) {  enableDisableTLSForward(tlsStatus: $tlsStatus) {  code  success  message  __typename  } } "}'

        ##DNS (addAccessControl for LAN01)
        curl -X POST http://127.0.0.1:8765/graphql -H "Content-Type:application/json" --max-time 600 -d '{"variables":{"AccessControl":[{"subnet":"'${DCLAN_DNS_AC}'","action":"allow","description":"'${DCLAN_INTERFACE}' subnet"}]}, "query":"mutation addAccessControl($AccessControl: [AccessControlInput!]!) {  addAccessControl(data: $AccessControl) {  code  success  message  __typename  } } "}'
        ...
        ##FILTER
        RES=$(curl -X POST http://127.0.0.1:8765/graphql -H "Content-Type:application/json" --max-time 600 -d '{ "query" : "query {  network {  getDNSFilterConfig { configStatus filterPending }  __typename  } }"}' | jq '.')
        if [[ "${RES}" =~ "ERROR" ]]; then
            echo -e "\nFailed in retrieving DNS Fitler configuration, Cannot proceed further\n";
        else
            DNS_FILTER_CONFIGURED=$(echo ${RES} | jq -r '.data.network.getDNSFilterConfig.configStatus' 2>/dev/null)
            if [ ${DNS_FILTER_CONFIGURED} == "false" ]; then
                ###set repo
                curl -X POST http://127.0.0.1:8765/graphql -H "Content-Type:application/json" --max-time 600 -d '{"variables":{"config":{"type":"refuse","urlPath":"'${DNS_FILTER_REPO}'","updateHour":-1,"configStatus":true,"defaultFilter":false}}, "query":"mutation saveDNSFilterConfig($config: DNSFilterConfigInput!) {  saveDNSFilterConfig(config: $config) {  code  message  success  __typename  } } "}'

                FILTER_PENDING=$(echo ${RES} | jq -r '.data.network.getDNSFilterConfig.filterPending' 2>/dev/null)
                while [ ${FILTER_PENDING} -eq 1 ]
                do
                    sleep 10
                    RES=$(curl -X POST http://127.0.0.1:8765/graphql -H "Content-Type:application/json" --max-time 600 -d '{ "query" : "query {  network {  getDNSFilterConfig { configStatus filterPending }  __typename  } }"}' | jq '.')
                    if [[ "${RES}" =~ "ERROR" ]]; then
                        echo -e "\nFailed in retrieving DNS Filter configuration, Cannot proceed further\n";
                        break
                    else
                        FILTER_PENDING=$(echo ${RES} | jq -r '.data.network.getDNSFilterConfig.filterPending' 2>/dev/null)
                    fi
                ...
                if [ ${FILTER_PENDING} -eq 0 ]; then
                    ###enable DNS filter from Steven Black and Adguard
                    curl -X POST http://127.0.0.1:8765/graphql -H "Content-Type:application/json" --max-time 600 -d '{"variables":{"ids":[3,4]}, "query":"mutation enableDNSFilterSource($ids: [Int!]!) {  enableDNSFilterSource(ids: $ids) {  code  success  message  __typename  } } "}'
                fi
            fi
        fi
	fi
